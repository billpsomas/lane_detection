...
class TUSIMPLE_AUG(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        if idx % 2 == 0:
            gt_image = cv2.imread(self.img_paths[int(idx/2)][0], cv2.IMREAD_UNCHANGED)
            gt_binary_image = cv2.imread(self.img_paths[int(idx/2)][1], cv2.IMREAD_UNCHANGED)
            gt_instance = cv2.imread(self.img_paths[int(idx/2)][2], cv2.IMREAD_UNCHANGED)
        else:
            gt_image = cv2.imread(self.img_paths[int((idx-1)/2)][0], cv2.IMREAD_UNCHANGED)
            gt_binary_image = cv2.imread(self.img_paths[int((idx-1)/2)][1], cv2.IMREAD_UNCHANGED)
            gt_instance = cv2.imread(self.img_paths[int((idx-1)/2)][2], cv2.IMREAD_UNCHANGED)
